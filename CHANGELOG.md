...
### Added
- **0.3.0-M5 Repository Freshness Capability & FreshnessToken SPI**:
  ...
  - Introduced `TemplateFreshnessProvider` capability interface (`io.github.minh124199.viettemplate.api.TemplateFreshnessProvider`) in `viet-template-api`, implemented across `ClasspathTemplateRepository`, `InMemoryTemplateRepository`, `FilesystemTemplateRepository`, and `CompositeTemplateRepository`. Added non-breaking default method `freshnessToken(TemplateId)` to `TemplateRepository`.

### Changed
- **0.3.0-M5 Generation-Aware Warmed-Lookup Simplification & Invariant Overhead Elimination**:
  - **Generation-Aware Cache Fast Path**: Refactored `VtlTemplateEngine.get(TemplateId)` to query `repository.freshnessToken(id)` and match against `PreparedTemplateEntry.freshnessToken()` and `GlobalMacroManager.macroGeneration()`. For unchanged templates and macros, the pre-instantiated canonical `Template` is returned immediately, eliminating source re-reading, SHA-256 fingerprinting, macro digest hashing, and key allocation on warm lookups.
  - **Global Macro Generation Tracking**: Replaced per-lookup global macro MD5/SHA digest hashing with an `AtomicLong macroGeneration` counter in `GlobalMacroManager`. Cache keys lazily compute and cache the macro digest string with double-checked locking only during initial template compilation.
  ...
  - **Cache Schema & Serialization Invariants**: Enforced strict `serialVersionUID = 1L` versioning invariants and defensive copying across cache record structures.
  - **JMH Performance Qualification**: Qualified on Java 21 and Java 25. Java 21 `warmedEngineGet` IR latency dropped by -14.12% (1255.46 -> 1078.15 ns/op) with -232 B/op allocation reduction (1504 -> 1272 B/op); `engineRenderRequest` REALISTIC IR dropped by -14.46% (2346.28 -> 2006.98 ns/op); Java 25 `warmedEngineGet` IR improved by -9.82% (1142.00 -> 1029.87 ns/op, -200 B/op); zero regression in retained template render. Recorded in [ADR-0018](docs/adr/0018-generation-aware-warmed-lookup.md).
