...
- **Milestone M6 (Runtime Exception Semantics, Failure Boundaries & Diagnostic Hardening — target 0.3.0-M6) — COMPLETE & FROZEN**:
    ...
    - **M6.7 (Documentation & ADR-0019)**: Formulated comprehensive architecture guide (`docs/architecture/exception-semantics.md`) and recorded ADR-0019 (`docs/adr/0019-runtime-exception-and-failure-boundaries.md`). Runtime failure boundary architecture is frozen.

- **Candidate Next Runtime Milestone (Reassessed from Current Architecture Debt)**:
  - *COMPILER_AOT_FORMAT_COMPATIBILITY* or *PUBLIC_API_1_0_READINESS*: Re-evaluate AOT class loading format versioning and stable 1.0 public API / SPI readiness.
  - *Engine Decomposition Status*: **NO_FURTHER_DECOMPOSITION**. `VtlTemplateEngine` (406 LOC) cleanly delegates to `AotTemplateRegistry`, `TemplateDependencyCoordinator`, and `TemplateCompilationCoordinator`.
...
- **Roadmap Sequence Towards 1.0 GA**:
  ...
  - **0.3.0 (Public Surface Encapsulation, Runtime Simplification & Exception Hardening)**: Execute Milestone M22 (public surface encapsulation), Milestones M4-M5 (runtime execution model, warmed-lookup simplification, M5.9 freeze), and Milestone M6 (runtime exception semantics and failure boundary hardening), securing binary compatibility and robust error handling before 1.0 GA.
