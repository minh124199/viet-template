...
- **Candidate Next Runtime Milestone (Reassessed from Current Architecture Debt)**:
  - *Runtime Exception Semantics Hardening & Boundary Audit*: Refine remaining broad catches (`catch (Exception/Throwable)`), enforce strict classification (`EXPECTED_MISS`, `IO_FAILURE`, `SECURITY`, `RECOVERABLE`), and eliminate bug-masking risks across interpreter and runtime reflection bridges.
...
- **Roadmap Sequence Towards 1.0 GA**:
  ...
  - **0.3.0 (Public Surface Encapsulation & Runtime Simplification Milestone)**: Execute Milestone M22 (public surface encapsulation) and Milestones M4-M5 (runtime execution model, warmed-lookup simplification, and M5.9 architecture freeze), securing binary compatibility and high performance before 1.0 GA.
