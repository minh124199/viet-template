...
- **Milestone M23 (Runtime Execution Model — target 0.3.0-M4+) — NEXT**:
    - **M23.0**: Baseline runtime audit (TemplateEngine.get, Template.render warm/cold paths, AOT path, IR path, AST fallback path)
    - **M23.1**: ExecutionTarget state analysis
    - **M4.5**: Benchmark relocation prototype (VariableAssignmentBenchmark, NestedScopeBenchmark, VariableLookupBenchmark) to a source set within viet-template-vtl-interpreter; JMH class-loading verification
    - **M23.5**: PreparedTemplateEntry / cache schema
    - **M23.6**: Engine decomposition + schema versioning
    - **M23.7**: AST preparation + exception cleanup
    - **M23.8**: JMH/JFR qualification
