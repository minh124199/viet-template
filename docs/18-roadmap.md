...
- **Milestone M5 (Generation-Aware Warmed-Lookup Simplification Program — target 0.3.0-M5) — IN PROGRESS**:
    - **M5.0 (Documentation & Benchmark Baseline Alignment) — COMPLETE**: Synchronize benchmark evidence, record matched Java 21 baseline, and qualify runtime architectural findings.
    - **M5.1 (Repository Freshness Capability Model)**: Introduce `TemplateFreshnessProvider` and `FreshnessToken` SPI in `viet-template-api` for positive caching without stream reading or SHA-256 fingerprinting on immutable/versioned repositories.
    - **M5.2 (Generation-Aware Positive Cache Lookup)**: Validate template cache hits using freshness tokens and macro generation counters directly in `VtlTemplateEngine.get(id)`.
    - **M5.3 (Global Macro Generation Token)**: Monotonic generation tracking in `GlobalMacroManager` eliminating repeated macro registry hashing on warmed hits.
    - **M5.4 (CompileCacheKey Construction Streamlining)**: Precomputed fingerprint reuse and zero-allocation key lookup on active cache entries.
    - **M5.5 (Negative Cache Integration & TTL Eviction)**: Freshness-aware negative cache eviction and capacity bounds.
    - **M5.6 (Public Surface Classification & ABI Verification)**: Strict containment of new capability interfaces to stable SPI/API boundaries.
    - **M5.7 (Matched JMH & JFR Qualification)**: Target $\le 100$ B/op (approaching 0 B/op) on warmed `engine.get()` across Java 21 and 25.
    - **M5.8 (Documentation & ADR Convergence)**: Record ADR-0018 and finalize milestone reports.
...
- **Roadmap Sequence Towards 1.0 GA**:
  ...
  - **0.3.0 (Public Surface Encapsulation & Runtime Simplification Milestone)**: Execute Milestone M22 (public surface encapsulation) and Milestones M4-M5 (runtime execution model and warmed-lookup simplification), securing binary compatibility and high performance before 1.0 GA.
