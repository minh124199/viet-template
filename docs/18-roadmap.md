...
- **Milestone M22 (Public Surface Containment & Internal Architecture, target 0.3.0) — IN PROGRESS**:
    ...
    - **M22.2 (Internal Package Relocation — Category B, target 0.3.0-M2)**: Move 62 internal implementation types to dedicated `*.internal.*` packages across `language-vtl`, `runtime`, and `vtl-interpreter`.
    - **M22.3 (Cross-Module Internal SPI & ABI Hardening — Category C & F, target 0.3.0-M3)**: Define formal internal SPI contracts for the 159 cross-module internal types and stabilize the 4 true generated bytecode runtime ABI types (`BytecodeRuntimeBridge`, `DynamicCallSite`, `LinkerAccessPolicy`, `ForeachMetadata`).
    - **M22.4 (JPMS Qualified Exports & Encapsulation Decision Gate, target 0.3.0-M4)**: Evaluate introducing `module-info.java` descriptors with qualified exports post-package relocation (currently DEFERRED).
- **Roadmap Sequence Towards 1.0 GA**:
  ...
  - **0.3.0 (Public Surface Encapsulation Milestone)**: Execute Milestone M22 to relocate and encapsulate accidental public types (`config/api-baseline/accidental-public-types-inventory.json`), securing binary compatibility before 1.0 GA.
