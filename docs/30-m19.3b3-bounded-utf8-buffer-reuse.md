...
> All empirical benchmarks documented herein were executed on the reference workstation (Linux 7.2.4-3-cachyos x86_64, Intel Core i5-8350U @ 1.70GHz, 4 physical cores, 8 logical threads, 11 GiB RAM). Methodological controls adhere to protocol `VT-PERF-M19.3B3-BOUNDED-BUFFER-REUSE-1` (1 fork, 2 warmup iterations, 3 measurement iterations, 1s each) across OpenJDK 17 (`J17-G1`, canonical production baseline), OpenJDK 21 LTS (`J21-G1`), and OpenJDK 25 (`J25-G1`).
...
Key characteristics:
- **Allocation Frequency**: 1 array allocation ($8,200\text{ bytes}$ with object header and padding) per streaming render.
...
- **Disadvantages**: Severe allocation churn ($8,200\text{ B/render}$); lower throughput on tiny/medium templates due to GC allocation stalls and memory bandwidth saturation.
...
- **Exact Array Elimination**: Across all 8 end-to-end workloads, the difference between unpooled and pooled execution was exactly $8,200.0\text{ B/op}$ ($8,192\text{ bytes array payload} + 8\text{ bytes object header/padding}$).
...
5. **Empirical Justification**: Eliminates exactly 8,200 bytes per render, delivering up to **+160.0% speedup** on tiny templates, **+26.5% speedup** on medium templates, and **0 contention events** across all tested JDKs.
