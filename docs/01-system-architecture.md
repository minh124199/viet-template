...
10. **Java 21 Bytecode Target**: All production modules, plugins, and AOT-generated classes strictly target Java 21 bytecode (classfile major version 65) via `-release 21`, ensuring zero multi-release JARs (MRJARs) and single-version bytecode uniformity (ADR-0007, ADR-0011).
11. **Virtual Thread Concurrency Guarantees**: Core template execution is fully compatible with virtual threads (Project Loom). The engine enforces zero thread pinning on I/O (blocking operations avoid `synchronized` blocks and use `ReentrantLock` or non-blocking primitives where needed), thread-confinement of render contexts and execution frames, zero `ThreadLocal` state leaks, and non-closing output stream ownership (ADR-0013).
12. **Zero Preview Feature Invariant**: Published APIs, SPIs, and production runtime paths strictly disallow preview features (`--enable-preview`), ensuring stable execution on standard JVM production deployments (ADR-0014).
